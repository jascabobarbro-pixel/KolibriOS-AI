...
"""
    ...
    async def _system_monitor_loop(self) -> None:
        """Background task to monitor system state via gRPC."""
        while self._running:
            try:
                # Try to get real metrics from CND Orchestrator
                if self._cnd_channel:
                    await self._fetch_system_metrics_from_cnd()
                elif self._kernel_channel:
                    await self._fetch_system_metrics_from_kernel()
                else:
                    # Fallback to simulated data if no connection
                    await self._fetch_simulated_metrics()
    ...
    async def _fetch_system_metrics_from_cnd(self) -> None:
        """Fetch system metrics from CND Orchestrator via gRPC."""
        try:
            # Try to import CND protobuf definitions
            try:
                from cnd_orchestrator.protos import cnd_orchestrator_pb2, cnd_orchestrator_pb2_grpc

                stub = cnd_orchestrator_pb2_grpc.CndOrchestratorServiceStub(self._cnd_channel)
                response = await stub.GetSystemMetrics(
                    cnd_orchestrator_pb2.GetSystemMetricsRequest()
                )

                # Update system state with real data
                self.system_state.total_memory = response.total_memory
                self.system_state.used_memory = response.used_memory
                self.system_state.memory_utilization = response.memory_utilization
                self.system_state.total_cores = response.total_cores
                self.system_state.active_cores = response.active_cores
                self.system_state.cpu_utilization = response.cpu_utilization
                self.system_state.running_tasks = response.running_tasks
                self.system_state.pending_tasks = response.pending_tasks

                logger.debug("Fetched real metrics from CND Orchestrator")

            except ImportError:
                # Protobuf not generated - use direct gRPC call approach
                # Try to query cells directly for metrics
                await self._fetch_metrics_from_cells_directly()

        except Exception as e:
            logger.warning(f"Could not fetch metrics from CND: {e}")
            # Fallback to simulated
            await self._fetch_simulated_metrics()

    async def _fetch_metrics_from_cells_directly(self) -> None:
        """Fetch metrics directly from cells when CND protobuf not available."""
        import grpc
        ...
        # Query memory cells
        memory_cell_endpoints = self.config.communication.memory_cell_endpoints or []
        for endpoint in memory_cell_endpoints:
            try:
                channel = grpc.aio.insecure_channel(endpoint)
                # Use gRPC health check and reflection for basic info
                from grpc_health.v1 import health_pb2, health_pb2_grpc
                health_stub = health_pb2_grpc.HealthStub(channel)
                health = await health_stub.Check(health_pb2.HealthCheckRequest())

                if health.status == health_pb2.HealthCheckResponse.SERVING:
                    cell_count += 1
                    # Estimate metrics based on health
                    total_memory += 1024 * 1024 * 1024  # 1GB default per cell
                    used_memory += 512 * 1024 * 1024   # 512MB used

                channel.close()
            except Exception as e:
                logger.debug(f"Could not query memory cell at {endpoint}: {e}")

        # Query processor cells
        processor_cell_endpoints = self.config.communication.processor_cell_endpoints or []
        for endpoint in processor_cell_endpoints:
            try:
                channel = grpc.aio.insecure_channel(endpoint)
                from grpc_health.v1 import health_pb2, health_pb2_grpc
                health_stub = health_pb2_grpc.HealthStub(channel)
                ...
                if health.status == health_pb2.HealthCheckResponse.SERVING:
                    total_cores += 4  # Assume 4 cores per processor cell
                    active_cores += 2  # Assume 2 active
                    cpu_util += 30.0   # Assume 30% utilization

                channel.close()
            except Exception as e:
                logger.debug(f"Could not query processor cell at {endpoint}: {e}")

        if cell_count > 0 or total_cores > 0:
            self.system_state.total_memory = total_memory
            self.system_state.used_memory = used_memory
            self.system_state.memory_utilization = (used_memory / total_memory * 100) if total_memory > 0 else 0
            self.system_state.total_cores = total_cores
            self.system_state.active_cores = active_cores
            self.system_state.cpu_utilization = cpu_util / max(len(processor_cell_endpoints), 1)
            logger.debug("Fetched metrics directly from cells")
        else:
            await self._fetch_simulated_metrics()

    async def _fetch_system_metrics_from_kernel(self) -> None:
        """Fetch system metrics directly from Kernel via gRPC."""
        try:
            # Try to query kernel for system state
            from grpc_health.v1 import health_pb2, health_pb2_grpc

            health_stub = health_pb2_grpc.HealthStub(self._kernel_channel)
            health = await health_stub.Check(health_pb2.HealthCheckRequest())

            # If kernel is healthy, use reasonable defaults
            if health.status == health_pb2.HealthCheckResponse.SERVING:
                self.system_state.neural_scheduler_active = True
                logger.debug("Kernel is healthy, neural scheduler active")

        except Exception as e:
            logger.warning(f"Could not query kernel: {e}")
            await self._fetch_simulated_metrics()

    async def _fetch_simulated_metrics(self) -> None:
        """Fallback to simulated metrics when real data unavailable."""
        import random
        import psutil

        try:
            # Use actual system metrics from psutil as fallback
            self.system_state.total_memory = psutil.virtual_memory().total
            self.system_state.used_memory = psutil.virtual_memory().used
            self.system_state.memory_utilization = psutil.virtual_memory().percent
            self.system_state.total_cores = psutil.cpu_count()
            self.system_state.active_cores = psutil.cpu_count(logical=False)
            self.system_state.cpu_utilization = psutil.cpu_percent(interval=0.1)
            logger.debug("Using psutil for system metrics")
        except ImportError:
            # Final fallback to simulated values
            self.system_state.memory_utilization = random.uniform(30, 70)
            self.system_state.cpu_utilization = random.uniform(10, 50)
            logger.debug("Using simulated metrics (psutil not available)")
